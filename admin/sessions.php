    ...
    if (isset($_POST['add_session'])) {
        ...
        if($title) {
            try {
                ...
                $stmt = $pdo->prepare("INSERT INTO academic_sessions (Title, StartDate, EndDate, IsActive) VALUES (?, ?, ?, ?)");
                $stmt->execute([$title, $sd, $ed, $isActive]);
                ...
                <tbody class="divide-y divide-gray-100">
                    <?php foreach($sessions as $s): ?>
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="p-4 text-gray-700 text-sm"><?php echo $s['StartDate'] ? date('M d, Y', strtotime($s['StartDate'])) : '--'; ?></td>
                            <td class="p-4 text-gray-700 text-sm"><?php echo $s['EndDate'] ? date('M d, Y', strtotime($s['EndDate'])) : '--'; ?></td>
                            <td class="p-4">
                                <?php if($s['IsActive']): ?>
        ...
        <form method="POST" class="p-6">
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="date" name="start_date" class="w-full border border-gray-300 rounded px-3 py-2 outline-none focus:border-maroon focus:ring-1 focus:ring-maroon">
                </div>
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-1">End Date</label>
                    <input type="date" name="end_date" class="w-full border border-gray-300 rounded px-3 py-2 outline-none focus:border-maroon focus:ring-1 focus:ring-maroon">
                </div>
            </div>
